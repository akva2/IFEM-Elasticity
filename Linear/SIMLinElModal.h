...
public:
                              ...
                              const Vectors& mSol, bool, bool)
  {
    ...
    if (!this->assembleModalSystem(time,mSol,Rhs,
                                   ...
                                   Dim::myProblem->getIntegrationPrm(4) == 2.0))
      return false;
  ...
  virtual const Vectors& expandSolution(const Vectors& mSol)
  {
    if (!this->expandSolution(mSol,sol))
      sol.clear();

    // Swap back the to the full equation system data for postprocessing,
